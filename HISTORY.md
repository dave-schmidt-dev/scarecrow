...
- **Feedback-driven summary review.** Summary review no longer asks you to retype the entire body. The `edit` path now collects correction instructions and sends them back into the summarizer as review feedback so the draft is regenerated from the transcript. `Ctrl+C` during summary review now exits cleanly and records a canceled review instead of dumping a traceback. Regression tests cover feedback-driven regeneration and interrupt handling.
- **Post-call summary validation.** After normal summary generation, Scarecrow now opens summary review before task review. The raw draft is preserved as `summary.generated.md`, the accepted or reviewed result remains `summary.md`, and review metadata is written to `summary_review.json`.
- **Weekly report overhaul.** Weekly reports now use a local-LLM overview at the top, followed by grouped action items and per-session 1-2 sentence briefs. Added `--last-week` to `scripts/report.py` for automation that always targets the last completed ISO week.
- **Post-call task review.** After normal summary generation, Scarecrow now opens a dedicated task review step that lets you refine the task list with freeform feedback before finalizing `summary.md`. Reviewed tasks are persisted in `task_review.json` and reused by future reprocess runs.
