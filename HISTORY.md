...
- **MLX summarizer backend:** Added `mlx-vlm` as an alternative backend for post-session summarization, targeting Apple Silicon native inference with optional TurboQuant KV-cache compression. Install with `uv sync --extra mlx-summarizer`, set `SUMMARIZER_BACKEND = "mlx"` in config. Refactored summarizer to use a backend protocol (`_GgufBackend` / `_MlxBackend`) — existing GGUF path unchanged. Added `gemma4` model pattern for Gemma 4 GGUF models. Added `--backend gguf|mlx` flag to `resummarize.py`. New benchmark script `benchmarks/bench_summarizer.py` for side-by-side comparison.
